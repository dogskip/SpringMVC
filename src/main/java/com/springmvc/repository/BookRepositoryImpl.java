	 ...
	 //생성자로 호출될 때 자동실행
		...
		public List<Book> getBookListByCategory(String category) {
			...
			for(int i=0;i<listOfBooks.size();i++) {
				Book book = listOfBooks.get(i); //객체가 들어감
				if(category.equalsIgnoreCase(book.getCategory()))
				//대소문자를 무시하고 문자열을 비교함. book.getCategory()의 값이 category와
				//같으면 list에 책정보 add
		...
		public Set<Book> getBookListByFilter(Map<String, List<String>> filter) {
	        ...
	        //매트릭스 변수 중 publisher를 포함하는 경우 실행
	        ...
	        //매트릭스변수 중 category를 포함하는 경우 실행되고, 전체 도서 목록 중 category값과 일치하는 도서를 검색하여 booksByCategory 객체에 등록
	        ...
	        //booksByCategory객체에 등록된 도서와 booksByPublisher에 등록된 도서 목록 중에서 중복되는 도서만 남기고 나머지는 삭제 후 booksByCategory 객체로 반환
